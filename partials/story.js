...
// Insert a small disclaimer above the video (first .video-wrap), if present
(function insertVideoDisclaimer(){
  try {
    const vid = root.querySelector('.video-wrap') || document.querySelector('.video-wrap');
    if (!vid) return;
    const prev = vid.previousElementSibling;
    if (prev && prev.getAttribute && prev.getAttribute('data-i18n') === 'video.disclaimer') return;
    const cap = document.createElement('div');
    cap.className = 'mb-2 text-xs text-gray-300';
    cap.setAttribute('data-i18n', 'video.disclaimer');
    cap.innerHTML = t('video.disclaimer', 'All footage in this video depicts real events and real people. The full meaning of these scenes becomes clear after reading our story.');
    vid.parentNode && vid.parentNode.insertBefore(cap, vid);
  } catch {}
})();
