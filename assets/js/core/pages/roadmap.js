...
// Вкладка "Хронология" (route: #/roadmap)
// Совместимо с архитектурой v200825-final: lazy-init, i18n, destroy(), без глобальных побочных эффектов.
...
let state = {
  locale: null,
  data: null,     // /partials/roadmap.json (скелет + факты)
  i18n: null,     // /i18n/Roadmap/<locale>.json (строки UI + подписи items)
};
...
function fmtDate(iso) {
  if (!iso) return '';
  try {
    // поквартально/помесячно — как в демо; пусть выбирается локаль браузера
...
// Слияние фактов из data с локализованными строками из i18n
function mergeLocalized(data, dict) {
  ...
  const mapItem = (it) => {
    ...
    if (loc) {
      ...
      // details по умолчанию остаётся из data; если захотим локализовывать — добавим позже
    }
    return it;
  };
...
function confidenceDots(it) {
  ...
  return ` · confidence <span class="roadmap-conf">${dots}</span>`;
}
...
function render(container, data, dict) {
  ...
  if (m && m[1]) {
    ...
    if (el) {
      ...
      // снять подсветку через пару секунд
...
function makeColumn(kind, titleText, list, opts) {
  ...
  // Слушатель аккордеона
  const clickHandler = (e) => {
    if (e.target.closest('.roadmap-toggle') !== toggle) return;
    const isOpen = !section.classList.contains('is-collapsed');
    section.classList.toggle('is-collapsed', isOpen);
    toggle.setAttribute('aria-expanded', String(isOpen ? false : true));
  };
  ...
  // Контент списка
  if (!list?.length) {
    const p = createEl('div', { class: 'roadmap-item' });
    p.textContent = tNoEntries || 'No entries yet.';
...
function makeCard(it, { tBtnDetails, tBtnHide, emphasizeNow }) {
  ...
  const when = createEl('div', { class: 'roadmap-when' });
  when.innerHTML = `${esc(it.when_label || labelFromDates(it))}${confidenceDots(it)}`;
  ...
  const summary = createEl('div', { class: 'roadmap-summary' });
  summary.textContent = it.summary || '';

  const details = createEl('div', { class: 'roadmap-details' });
  details.textContent = it.details || '';

  const actions = createEl('div', { class: 'roadmap-actions' });
  const btn = createEl('button', { class: 'roadmap-btn' });
  btn.textContent = tBtnDetails || 'Details';
  const onToggle = () => {
    const vis = details.style.display === 'block';
    details.style.display = vis ? 'none' : 'block';
    btn.textContent = vis ? (tBtnDetails || 'Details') : (tBtnHide || 'Hide');
  };
  ...
  // Пульс-индикатор у текущего шага (в заголовке колонки уже есть точка, но добавим и на карточке при необходимости)
...
function makeBadge(text) {
  ...
  const span = createEl('span', { class: 'roadmap-badge' + (
    ['done', 'current', 'planned', 'tentative'].includes(t) ? (' status-' + t) : ''
  ) });
...
async function loadAll() {
  ...
  if (!state.data) {
    // Факты/скелет
    state.data = await loadJSON('/partials/roadmap.json');
  }
  // Локализация UI+items
  state.i18n = await loadJSON(`/i18n/Roadmap/${locale}.json`).catch(() => ({}));
...
export async function init({ mount } = {}) {
  ...
  // Собираем собственную "подложку" секции, чтобы не зависеть от внешних partial-ов
  ...
  // Навешиваем делегирование кликов только для совместимости (не обязательно)
  const offDoc = on(document, 'click', (e) => {
    // здесь можно ловить доп. взаимодействия, если появятся
    return;
  });
  cleanup.push(offDoc);
}
