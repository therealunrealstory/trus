...
// Medical Timeline в двух контейнерах (как Legal): .roadmap-head + .roadmap-body.
// Работает как модуль для вкладки «Timeline» (монтируется в #medical-timeline).
...
const getLocale = I18N.getLocale || I18N.getLangFromQuery || (() => document.documentElement.lang || 'en');
...
function statusLabel(s){
  return (s==='done')?'done':(s==='current')?'current':(s==='planned')?'planned':(s==='tentative')?'tentative':(s||'');
}
...
function pretty(t){
  const map = { 'rt':'RT','car-t':'CAR-T','tki':'TKI','mi':'MI' };
...
function makeCard(it, { tBtnDetails, tBtnHide }){
  ...
  when.innerHTML = `${esc(it.when_label || labelFromDates(it))}${confidenceDots(it, state.i18n)}`;
...
function makeColumn(kind, titleText, list, opts){
  ...
  cleanup.push(() => header.removeEventListener('click', onClick));
  ...
  if (!list?.length) {
    ...
    list.forEach(it => listEl.appendChild(makeCard(it, { tBtnDetails, tBtnHide })));
  }
...
function render(container, data, dict, { medicalWrap = true } = {}){
  container.innerHTML = '';
  ...
  // --- Header card ---
  const head = createEl('div', { class: 'roadmap-head' });
  ...
  // --- Body card ---
  const body = createEl('div', { class: 'roadmap-body' });
  ...
  // anchor #/roadmap/<id> — раскрыть колонку и подсветить карточку
...
export async function init(mountArg){
  ...
  const root = mount || qs('#subpage'); // на вкладке Timeline сюда передадут #medical-timeline
  if (!root) return;
  ...
  // По умолчанию medicalWrap = true (двухконтейнерная схема).
  // Если вдруг модуль переиспользуют на отдельной странице Roadmap — можно передать { medicalWrap:false } в render.
  render(page, merged, state.i18n, { medicalWrap: true });

  const unSub = onLocaleChanged(async () => {
    const merged2 = await loadAll();
    render(page, merged2, state.i18n, { medicalWrap: true });
  });
  cleanup.push(unSub);
