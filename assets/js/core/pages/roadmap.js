...
// Вкладка «Хронология»: совместимо с твоим кодом, локализует детали/бейджи/“confidence”,
// путь локалей строго строчный: /i18n/roadmap/<lc>.json
...
function card(it, dict, emphasizeNow=false){
  ...
  const meta=createEl('div',{class:'roadmap-card-meta'});
  ...
  wrap.append(top, createEl('div',{class:'roadmap-row'})
    , date
    , summary
    , (it.details?details:null)
    , actions
  );
...
function render(root, data, dict){
  ...
  const hDone   = createEl('h3', { 'data-i18n': 'ui.section_completed' }); hDone.textContent = dict?.ui?.section_completed || 'Completed';
  ...
  const hNow   = createEl('h3', { 'data-i18n': 'ui.section_now' }); hNow.textContent = dict?.ui?.section_now || 'Now';
  const pulse  = createEl('span', { class: 'roadmap-nowpulse', title: 'Now' });
  hNow.append(' ', pulse);
  ...
  const hPlan   = createEl('h3', { 'data-i18n': 'ui.section_plans' }); hPlan.textContent = dict?.ui?.section_plans || 'Plans';
...
export async function init(mountArg){
  let mount = mountArg && typeof mount === 'object' && 'mount' in mount ? mountArg.mount : mountArg;
