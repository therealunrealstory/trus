...
// Медицинская хронология: шапка + тело (3 раскрывающихся списка), как у Legal.
...
const getLocale = I18N.getLangFromQuery || I18N.getLocale || (() => (document.documentElement.lang || 'EN'));
...
function tLabel(key, dict){ const k=(key||'').toLowerCase(); return dict?.labels?.[k] || pretty(k); }
function labelFromDates(it){
  const a = fmtDate(it.date_start); const b = fmtDate(it.date_end);
  if (a && b) return `${a} – ${b}`;
...
function render(container, data, dict){
  ...
  // Якорь вида #/roadmap/<id> — подсветка карточки
...
export async function init(mountArg){
  ...
  const root = mount || qs('#subpage'); // на /timeline придёт элемент #medical-timeline
  if (!root) return;
