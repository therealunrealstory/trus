...
function parseDonateConfigFromDOM(root){
  ...
  const boostyUrl = (wrap.getAttribute('data-boosty-url') || '').trim();

  // Dev override via URL or localStorage (?donate=glocal|gofundme|boosty)
  try {
    ...
    if (override === 'glocal' || override === 'gofundme' || override === 'boosty') provider = override;
  } catch {}
  ...
  return { provider, base, amounts, currency, boostyUrl, wrap };
}
...
function renderDonateButtons(root){
  ...
  const { provider, base, amounts, currency, boostyUrl, wrap } = cfg;
  wrap.innerHTML = '';

  // ——— BOOSTY ———
  if (provider === 'boosty') {
    const url = boostyUrl || 'https://boosty.to/theRealUnrealStoryNico';
    const a = document.createElement('a');
    a.className = 'donate-tier px-4 py-3 rounded-2xl text-sm';
    a.target = '_blank';
    a.rel = 'noopener noreferrer';
    a.href = url;
    a.setAttribute('data-i18n','btn.boosty');
    a.textContent = t('btn.boosty','Support on Boosty');
    wrap.appendChild(a);

    const note = document.createElement('p');
    note.className = 'mt-2 text-xs text-gray-300';
    note.setAttribute('data-i18n','donate.note.boosty');
    note.textContent = t('donate.note.boosty','You will be redirected to boosty.to for a secure contribution.');
    ...
    infoBtn.addEventListener('click', ()=>{
      ...
      const html = t('donate.info.body', `<div class="space-y-3 text-sm leading-relaxed">
        ...
        <p><strong>Your voluntary contribution matters:</strong> it supports work on the project (research, texts, translations, visuals and audio, publishing, hosting, site maintenance) and <strong>helps cover expenses that arise directly from this story.</strong></p>
        <p><strong>What’s next — learning & tools.</strong> Practical courses and my own web-apps with unique AI-driven methods (some features free). Stay tuned for updates and releases here.</p>
      </div>`);
