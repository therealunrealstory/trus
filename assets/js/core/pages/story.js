...
import { $ } from '../dom.js';
import { t, I18N, DEFAULT_I18N, onLocaleChanged, applyI18nTo } from '../i18n.js';
...
/* ====== Nico WhatsApp contact block under Comment ====== */
let storyContactMO = null;
const STORY_CONTACT_ID = 'story-nico-contact';
function storyContactHTML(){
  // Provide default text for i18n fallback via DEFAULT_I18N capture
  const p = document.createElement('p');
  p.className = 'indent';
  p.setAttribute('data-i18n','contact.nico.html');
  p.innerHTML = '— <strong>Nico:</strong> I’m here and open to talk. WhatsApp: <a href="https://wa.me/393515160791" target="_blank" rel="noopener" class="underline decoration-cyan-500/60 hover:no-underline focus:no-underline focus:ring-2 focus:ring-cyan-500/40">+39 351 516 0791</a>';
  const wrap = document.createElement('div');
  wrap.className = 'rtxt';
  wrap.appendChild(p);
  const sec = document.createElement('section');
  sec.id = STORY_CONTACT_ID;
  sec.appendChild(wrap);
  return sec;
}
function ensureContactAfterComment(){
  const anchor = document.getElementById('comment-block');
  const container = document.getElementById('subpage') || document.querySelector('main') || document.body;
  if (!container) return;
  let sec = document.getElementById(STORY_CONTACT_ID);
  if (!sec) sec = storyContactHTML();
  // Prefer to insert right after Comment block if present; else after the first section as a fallback
  ...
  // Apply i18n to the newly inserted/updated section
  try { applyI18nTo(sec); } catch {}
}
function watchContactOrder(){
  // Observe mutations and keep the contact block right under Comment if Comment re-renders
  const target = document.getElementById('subpage') || document.body;
  if (!target) return;
  if (storyContactMO) try{ storyContactMO.disconnect(); }catch{}
  storyContactMO = new MutationObserver(() => {
    // Throttle microburst by using requestAnimationFrame
    requestAnimationFrame(()=> ensureContactAfterComment());
  });
  storyContactMO.observe(target, { childList: true, subtree: true });
}
/* ------------------------------------------------------- */
...
export function destroy(){
  ...
  try { storyContactMO?.disconnect(); } catch {}
}
